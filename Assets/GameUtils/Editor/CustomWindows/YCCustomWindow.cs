...
#else
                EditorWindow[] filteredWindows = Resources.FindObjectsOfTypeAll<EditorWindow>().Where(window => window.docked && window.hasFocus).ToArray();
                float minX = float.MaxValue;
                float minY = float.MaxValue;
                foreach (EditorWindow window in filteredWindows) {
                    if (window.position.x < minX) {
                        minX = window.position.x;
                    }
                    if (window.position.y < minY) {
                        minY = window.position.y;
                    }
                }
                float width = 0;
                float height = 0;
